...
const compareNums = (num1, num2) => {
  const numDiff = num1 - num2 >= 0 ? num1 : num2
  console.log(numDiff)
}
...
console.log(bestMovie(`American Dream`))
...
function upperCaseConverter (e){
  let upperString = e.toUpperCase()
...
function emailCheck(email){
  console.log(`${(String(email).trim()).includes(`@`) ? `email verified`: `must provide a valide email address`}`)
}

emailCheck(`lk@gmail.com`)
...
function frogsInThePocket (moneyLimit){
  let totalFrogs = moneyLimit/3
...
function frogsInThePocket2 (moneyLimit){
  let totalFrogs = Math.floor(moneyLimit/3)
  console.log(totalFrogs)
}

frogsInThePocket2(10)
...
function blah (sortArray){
  for(i=0; i<sortArray.length; i++){
    if(sortArray[i] >= sortArray[i+1]){
      console.log(`false`)
      break
    } else if (i=sortArray.length){
      console.log(`true`)
    }
  }
}

arrayIsAscending = blah(sampleArray)
